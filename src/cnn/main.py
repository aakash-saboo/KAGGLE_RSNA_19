...
def train(cfg, model):
    ...
    if cfg.resume_from:
        detail = util.load_model(cfg.resume_from, model, optim=optim,new_lr_bool=cfg.new_lr_bool,new_lr=cfg.new_lr)
        best.update({
            'loss': detail['loss'],
            'score': detail['score'],
            'epoch': detail['epoch'],
    ...
    if cfg.apex:
        ...
        print("optim learning rate from train function before amp initialisation =",optim.param_groups[0]['lr'])

        amp.initialize(model, optim, opt_level='O1')

        print("optim learning rate from train function=",optim.param_groups[0]['lr'])

        if cfg.use_multi_GPU:
            model = nn.DataParallel(model)
            log("MODEL is DATAPARALLEL MODEL")
